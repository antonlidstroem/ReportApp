...
public class SyncfusionExcelGenerator : IReportGenerator
{
    public async Task<byte[]> GenerateAsync(ReportDataViewModel data)
    {
        ...
        // Use Color.FromArgb instead of ExcelKnownColors enum to avoid variant-name
        // differences between Syncfusion versions (Navy/Dark_blue/DarkBlue vary by release).
        sheet.Range["A1"].CellStyle.Font.RGBColor = Color.FromArgb(255, 15, 45, 74);

        sheet.Range["A2"].Text = $"{data.CompanyName}  ·  Genererad {data.GeneratedAt:yyyy-MM-dd}";
        sheet.Range["A2"].CellStyle.Font.RGBColor = Color.FromArgb(255, 100, 116, 139);
