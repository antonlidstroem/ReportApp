...
public class SyncfusionPdfGenerator : IReportGenerator
{
    public async Task<byte[]> GenerateAsync(ReportDataViewModel data)
    {
        ...
        // NOTE: PdfGridRow.ApplyStyle() takes PdfGridCellStyle, not PdfGridRowStyle.
        // The header font/colour is applied per-cell in the loop below — no row-level call needed.
        for (int i = 0; i < 4; i++)
        {
