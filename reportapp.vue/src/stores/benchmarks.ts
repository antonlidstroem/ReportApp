...
export interface BenchmarkResult {
  provider: string; format: string; surveyId: number; surveyTitle: string
  generationMs: number; fileSizeBytes: number; timestamp: string; questionCount: number
}
...
export const useBenchmarkStore = defineStore('benchmarks', () => {
  ...
  function clear() { results.value = []; localStorage.removeItem(STORAGE_KEY) }
