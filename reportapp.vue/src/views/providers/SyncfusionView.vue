  ...
  <div class="page">
    ...
    <section class="section">
      ...
      <div class="formula-sandbox">
        ...
        <div class="fs-code">
          ...
          <div class="code-body"><pre><code>{{ formulaCode }}</code></pre></div>
        </div>
      </div>
    </section>
    ...
    <section class="section">
      ...
      <div class="chart-demo">
        ...
        <div class="fs-code">
          ...
          <div class="code-body"><pre><code>{{ nativeChartCode }}</code></pre></div>
        </div>
      </div>
    </section>
    ...
    <section class="section">
      ...
      <div class="sandbox-grid">
        <div v-for="f in sandboxFeatures" :key="f.id" class="sc" :class="{ active: (config as any)[f.id] }" @click="(config as any)[f.id] = !(config as any)[f.id]">
          <div class="sc-top"><span class="sc-icon">{{ f.icon }}</span><div class="sc-toggle" :class="{ on: (config as any)[f.id] }"><div class="sc-thumb"></div></div></div>
          <div class="sc-title">{{ f.title }}</div>
          <div class="sc-desc">{{ f.desc }}</div>
          <div class="sc-snippet"><code>{{ f.snippet }}</code></div>
          <div v-if="(config as any)[f.id]" class="sc-badge">✓ Active on next export</div>
        </div>
      </div>
    </section>
...
<script setup lang="ts">
  ...
  const rowAvg = (r: any) => (r.q1 + r.q2 + r.q3) / 3
...
<style scoped>
  ...
  .hero-bg {
    position: absolute;
    inset: 0;
    background: radial-gradient(ellipse at 80% 50%,rgba(20,184,166,.15) 0%,transparent 70%)
  }
  ...
  .hero-products {
    ...
    border-left: 1px solid rgba(20,184,166,.1)
  }
  ...
  .fs-head {
    display: grid;
    grid-template-columns: 36px 130px 1fr 1fr 1fr 160px;
    background: #0a2228;
    border-bottom: 1px solid rgba(20,184,166,.15)
  }
  ...
  .fs-row {
    display: grid;
    grid-template-columns: 36px 130px 1fr 1fr 1fr 160px;
    border-bottom: 1px solid rgba(20,184,166,.06);
    cursor: pointer;
    ...
    .fs-row.fs-total {
      background: rgba(20,184,166,.05);
      border-top: 2px solid rgba(20,184,166,.2)
    }
