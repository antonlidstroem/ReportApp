  ...
  <div class="dashboard">
        ...
        >
          ...
          <div class="pc-scores">
            <div v-for="s in p.scores" :key="s.l" class="pcs-row">
              ...
              <div class="pcs-bar"><div class="pcs-fill" :style="`width:${s.v*10}%;background:${p.color}`"></div></div>
            </div>
          </div>
          <div class="pc-footer">
            ...
            <template v-for="(group, gi) in filteredMatrix" :key="group.category">
              ...
              <tr v-for="(row, ri) in group.rows" :key="ri" class="data-row">
                ...
                <td v-for="(val, vi) in row.values" :key="vi" class="feature-cell">
                  ...
                  <span v-else-if="typeof val === 'number'" class="score-chip" :style="`--s:${val}`">{{ val }}/10</span>
    ...
    <section class="section">
      ...
      <div v-if="!benchmarks.results.length" class="empty-bench">
        ...
        <div class="eb-sub">Gå till en provider-sida och exportera en rapport för att se prestandadata här.</div>
      </div>
        ...
        <div class="bench-table-card">
          ...
          <div class="bench-table">
            <div class="bt-row header">
              <span>Provider</span><span>Format</span><span>Tid</span><span>Storlek</span><span>Frågor</span>
            </div>
            <div v-for="r in benchmarks.results.slice(0, 10)" :key="r.timestamp" class="bt-row">
              ...
              <span class="bt-time" :class="speedClass(r.generationMs)">{{ r.generationMs }}ms</span>
    ...
    <section class="section">
      ...
      <div class="decision-grid">
        <div v-for="d in decisionGuide" :key="d.scenario" class="decision-card" :style="`--dc:${d.color}`">
    ...
    <section class="section stress-section">
      <div class="stress-inner">
        ...
        <div class="stress-btns">
          <router-link v-for="p in providers" :key="p.route" :to="p.route" class="stress-btn" :style="`--c:${p.color}`">
            {{ p.icon }} {{ p.shortName }}
          </router-link>
        </div>
      </div>
...
const fullMatrix = [
  ...
  },
  {
    ...
    rows: [
      ...
      { feature: 'Radar/donut/linje-chart i PDF', values: ['Manuellt', 'Via HTML', 'Chart.js', 'Manuellt'] },
    ]
  },
  {
...
const radarDims = ['PDF-kvalitet', 'Excel', 'PPT', 'Grafer', 'Kostnad/licens', 'HTML-mallar', 'Enterprise', 'Prestanda']
...
const decisionGuide = [
  { icon: '💰', scenario: 'Budget är viktigast', winner: 'QuestPDF + ClosedXML', reason: 'Noll licenskostnad för Community. Alla tre bibliotek är MIT.', color: '#22c55e' },
  { icon: '🎨', scenario: 'Frontend-designern ska bygga mallen', winner: 'IronSuite eller jsreport', reason: 'Båda stödjer HTML/CSS-mallar. Designern arbetar i sitt rätta medium.', color: '#3b82f6' },
  { icon: '📊', scenario: 'Chart.js-grafer direkt i PDF', winner: 'jsreport', reason: 'Enda providern som kör JavaScript (Chart.js) inuti Chromium vid rendering.', color: '#a855f7' },
  { icon: '🧮', scenario: 'Excel-formulär som räknar om sig', winner: 'Syncfusion XlsIO', reason: '400+ native Excel-formler. Mottagaren öppnar Excel — data räknas om automatiskt.', color: '#14b8a6' },
  { icon: '🔐', scenario: 'Kryptering, signaturer, PDF/UA', winner: 'Syncfusion eller IronSuite', reason: 'Båda erbjuder enterprise-grade PDF-säkerhet. Syncfusion leder på accessibility.', color: '#f59e0b' },
  { icon: '⚡', scenario: 'Snabbast möjliga rendering', winner: 'QuestPDF / Syncfusion', reason: 'Inga externa processer. Ren .NET — renderingstid under 100ms för normala rapporter.', color: '#ef4444' },
]
...
watch(() => benchmarks.results.length, () => {
...
.dashboard { font-family: 'Cabinet Grotesk', sans-serif; max-width: 1300px; margin: 0 auto; color: #e2e8f0; }
...
.dh-grid { position: absolute; inset: 0; background-image: linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px); background-size: 50px 50px; }
.dh-glow { position: absolute; width: 600px; height: 600px; background: radial-gradient(circle, rgba(99,102,241,0.15) 0%, transparent 70%); top: -200px; right: -100px; }
.dh-content { position: relative; z-index: 1; }
.dh-tag { font-family: 'Roboto Mono', monospace; font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: rgba(255,255,255,0.4); margin-bottom: 16px; }
.dash-hero h1 { font-size: 52px; font-weight: 900; letter-spacing: -2px; margin: 0 0 12px; color: #fff; }
.dash-hero p { font-size: 18px; color: rgba(255,255,255,0.5); margin: 0 0 36px; }
.dh-meta { display: flex; gap: 16px; flex-wrap: wrap; }
.meta-pill { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 12px 20px; text-align: center; }
.mp-val { display: block; font-size: 24px; font-weight: 800; color: #fff; font-family: 'Roboto Mono', monospace; }
.mp-lbl { font-size: 11px; color: rgba(255,255,255,0.4); text-transform: uppercase; letter-spacing: 1px; }
...
.provider-card:hover { transform: translateY(-3px); box-shadow: 0 16px 40px rgba(0,0,0,0.3); border-color: var(--c); }
.pc-top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
.pc-icon { font-size: 26px; }
.pc-track { font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: var(--c); font-family: 'Roboto Mono', monospace; }
.provider-card h3 { font-size: 14px; font-weight: 700; margin: 0 0 8px; color: #fff; }
.provider-card p { font-size: 12px; color: #64748b; line-height: 1.5; flex: 1; margin: 0 0 12px; }
.pc-tags { display: flex; flex-wrap: wrap; gap: 5px; margin-bottom: 14px; }
.tag { font-size: 10px; padding: 2px 8px; background: color-mix(in srgb, var(--c) 10%, transparent); color: var(--c); border-radius: 100px; border: 1px solid color-mix(in srgb, var(--c) 25%, transparent); }
.pc-scores { display: flex; flex-direction: column; gap: 6px; margin-bottom: 14px; }
.pcs-row { display: flex; align-items: center; gap: 8px; }
.pcs-l { font-size: 10px; color: #64748b; width: 60px; flex-shrink: 0; }
.pcs-bar { flex: 1; height: 4px; background: #1e293b; border-radius: 2px; overflow: hidden; }
.pcs-fill { height: 100%; border-radius: 2px; }
.pc-footer { display: flex; justify-content: space-between; align-items: center; padding-top: 12px; border-top: 1px solid rgba(255,255,255,0.05); }
.pc-cost { font-size: 12px; font-weight: 700; }
.pc-cost.free { color: #22c55e; }
.pc-cost.paid { color: #f59e0b; }
.pc-cost.freemium { color: #a855f7; }
.pc-arrow { color: #475569; transition: transform 0.15s; }
.provider-card:hover .pc-arrow { transform: translateX(4px); color: var(--c); }

/* Matrix */
.matrix-filters { display: flex; gap: 8px; margin-bottom: 16px; flex-wrap: wrap; }
.mf-btn { font-size: 12px; padding: 6px 16px; border-radius: 100px; border: 1px solid #1e293b; background: transparent; color: #64748b; cursor: pointer; transition: all 0.15s; font-family: 'Cabinet Grotesk', sans-serif; }
.mf-btn:hover { border-color: #475569; color: #94a3b8; }
.mf-btn.active { background: #0f172a; border-color: #475569; color: #e2e8f0; }
.matrix-wrap { overflow-x: auto; border-radius: 12px; border: 1px solid #1e293b; }
.matrix { width: 100%; border-collapse: collapse; }
.matrix th { background: #0a1628; padding: 14px 16px; text-align: left; font-size: 13px; font-weight: 700; border-bottom: 2px solid #1e293b; white-space: nowrap; }
.matrix th:first-child { color: #94a3b8; }
.mh-inner { display: flex; align-items: center; gap: 6px; }
.mh-icon { }
.category-row .cat-header { background: #0a1628; padding: 8px 16px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: #94a3b8; }
.data-row:hover td { background: rgba(255,255,255,0.02); }
.matrix td { padding: 10px 16px; border-bottom: 1px solid #0f172a; font-size: 12px; }
.feature-name { color: #94a3b8; font-weight: 500; }
.feature-col { width: 240px; }
.feature-cell { text-align: center; }
.check { color: #22c55e; font-size: 14px; font-weight: 700; }
.cross { color: #ef4444; font-size: 14px; }
.partial { font-size: 11px; color: #94a3b8; }
.score-chip { font-size: 11px; font-weight: 700; padding: 2px 6px; border-radius: 4px; background: rgba(255,255,255,0.05); }

/* Radar section */
.radar-section { display: flex; gap: 24px; align-items: flex-start; }
.radar-charts { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; flex: 1; }
.radar-wrap { background: #050d1f; border-radius: 10px; padding: 16px; border: 1px solid #1e293b; }
.rw-title { font-size: 13px; font-weight: 700; margin-bottom: 8px; }
.radar-legend { width: 160px; flex-shrink: 0; background: #050d1f; border: 1px solid #1e293b; border-radius: 10px; padding: 16px; }
.rl-row { display: flex; align-items: center; gap: 8px; font-size: 11px; color: #64748b; padding: 4px 0; }
.rl-dot { width: 6px; height: 6px; border-radius: 50%; background: #475569; flex-shrink: 0; }

/* Benchmark */
.bench-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
.bench-chart-card { background: #050d1f; border: 1px solid #1e293b; border-radius: 12px; padding: 20px; grid-column: 1; }
.bcc-header { font-size: 13px; font-weight: 700; margin-bottom: 12px; color: #94a3b8; }
.bench-stats { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; grid-column: 2; grid-row: 1 / 3; align-content: start; }
.bench-table-card { background: #050d1f; border: 1px solid #1e293b; border-radius: 12px; overflow: hidden; grid-column: 1; }
.btc-header { font-size: 13px; font-weight: 700; padding: 14px 16px; background: #0a1628; border-bottom: 1px solid #1e293b; color: #94a3b8; }
.bench-table { }
.bt-row { display: grid; grid-template-columns: 110px 60px 70px 70px 50px; padding: 8px 16px; border-bottom: 1px solid #0f172a; font-size: 12px; }
.bt-row.header { font-size: 11px; color: #475569; background: #050d1f; font-weight: 700; text-transform: uppercase; }
.bt-provider { color: #94a3b8; font-weight: 600; }
.bt-format { color: #64748b; font-family: 'Roboto Mono', monospace; font-size: 10px; }
.bt-time.fast { color: #22c55e; font-weight: 700; }
.bt-time.ok { color: #f59e0b; font-weight: 700; }
.bt-time.slow { color: #ef4444; font-weight: 700; }
.bt-size { color: #64748b; font-size: 11px; }
.bt-q { color: #475569; font-size: 11px; }
.bs-card { background: #050d1f; border: 1px solid #1e293b; border-radius: 10px; padding: 16px; }
.bs-val { font-size: 22px; font-weight: 800; font-family: 'Roboto Mono', monospace; }
.bs-lbl { font-size: 11px; color: #64748b; margin-top: 4px; }
.empty-bench { padding: 48px; text-align: center; background: #050d1f; border: 1px dashed #1e293b; border-radius: 12px; }
.eb-icon { font-size: 40px; margin-bottom: 12px; }
.eb-title { font-size: 16px; font-weight: 700; color: #e2e8f0; margin-bottom: 6px; }
.eb-sub { font-size: 13px; color: #475569; }
.clear-btn { font-size: 12px; padding: 6px 14px; border-radius: 6px; border: 1px solid #1e293b; background: transparent; color: #64748b; cursor: pointer; }
...
.decision-card { background: #050d1f; border: 1px solid rgba(255,255,255,0.05); border-left: 4px solid var(--dc); border-radius: 12px; padding: 20px; }
.dc-icon { font-size: 24px; margin-bottom: 8px; }
.dc-scenario { font-size: 13px; font-weight: 700; color: #e2e8f0; margin-bottom: 6px; }
.dc-winner { font-size: 13px; font-weight: 700; color: var(--dc); margin-bottom: 6px; }
.dc-reason { font-size: 12px; color: #64748b; line-height: 1.5; }

/* Stress */
.stress-section { background: #050d1f; border: 1px solid #1e293b; border-radius: 16px; padding: 32px; }
.stress-inner { display: flex; justify-content: space-between; align-items: center; gap: 24px; flex-wrap: wrap; }
.stress-section h2 { font-size: 20px; font-weight: 800; margin: 0 0 8px; }
.stress-section p { font-size: 13px; color: #64748b; margin: 0; }
.stress-btns { display: flex; gap: 10px; flex-wrap: wrap; }
.stress-btn { padding: 10px 20px; background: color-mix(in srgb, var(--c) 15%, transparent); color: var(--c); border: 1px solid color-mix(in srgb, var(--c) 30%, transparent); border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 13px; transition: all 0.15s; }
.stress-btn:hover { background: var(--c); color: white; }
</style>
