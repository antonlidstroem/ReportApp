...
export async function exportReport(params: ExportParams): Promise<ExportResult> {
  ...
  try {
    ...
    if (err instanceof AxiosError && err.response?.data instanceof Blob) {
      ...
      throw new Error(`Export misslyckades (${err.response.status}): ${t.substring(0, 300)}`);
    }
    throw err;
  }

  const serverMs = res.headers["x-generation-time-ms"];
  const generationMs = serverMs ? parseInt(serverMs) : Math.round(performance.now() - t0);
  const fmt = params.format.toLowerCase();
  const ext = fmt === "excel" ? "xlsx" : fmt === "ppt" ? "pptx" : fmt;

  return {
    blob: res.data,
    filename: `rapport_${params.provider}_${fmt}_${new Date().toLocaleDateString("sv-SE").replace(/-/g,"")}.${ext}`,
    generationMs,
...
<div class="cover">
  ...
  <div class="cover-meta">Genererad {{GeneratedAt}}{{#if StartDate}} · Period: {{StartDate}}–{{EndDate}}{{/if}}</div>
</div>
<div class="kpi-strip">
...
<div class="section">
  ...
  <table>
    <thead><tr><th>Fråga</th><th>Kategori</th><th>Snitt</th><th>Svar</th><th style="width:15%">Fördelning</th></tr></thead>
    <tbody>
      {{#each QuestionSummaries}}
      <tr>
        <td>{{Text}}</td>
        <td><span class="cat-badge">{{Category}}</span></td>
        <td class="score {{#if (gt AverageValue 4)}}score-high{{else}}{{#if (gt AverageValue 3)}}score-med{{else}}score-low{{/if}}{{/if}}">{{AverageValue}}</td>
        <td>{{TotalResponses}}</td>
        <td><div class="bar-bg"><div class="bar-fill" style="width:{{AverageValue20Pct}}%;background:{{#if (gt AverageValue 4)}}#16a34a{{else}}{{#if (gt AverageValue 3)}}#d97706{{else}}#dc2626{{/if}}{{/if}}"></div></div></td>
      </tr>
      {{/each}}
    </tbody>
  </table>
...
{{#if Trends}}
<div class="section">
  <div class="section-title">Månadsvis trend</div>
  <div class="chart-wrap"><canvas id="l"></canvas></div>
</div>
{{/if}}
...
const bColors=bData.map(v=>v>=4?'#16a34a':v>=3?'#d97706':'#dc2626');
new Chart(document.getElementById('b'),{type:'bar',data:{labels:bLabels,datasets:[{data:bData,backgroundColor:bColors,borderRadius:4,borderSkipped:false}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{min:0,max:5,ticks:{stepSize:1},grid:{color:'#f1f5f9'}},x:{grid:{display:false}}}}});
{{#if Trends}}
const lLabels=[{{#each Trends}}'{{MonthName}} {{Year}}'{{#unless @last}},{{/unless}}{{/each}}];
const lData=[{{#each Trends}}{{AverageValue}}{{#unless @last}},{{/unless}}{{/each}}];
new Chart(document.getElementById('l'),{type:'line',data:{labels:lLabels,datasets:[{data:lData,borderColor:'#1e3a5f',backgroundColor:'rgba(30,58,95,.08)',fill:true,tension:0.4,pointRadius:5,pointBackgroundColor:'#1e3a5f'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{min:0,max:5,grid:{color:'#f1f5f9'}},x:{grid:{display:false}}}}});
{{/if}}
</script></body></html>`,
...
.score-num{font-size:11px;font-weight:700;width:28px;text-align:right}
...
<div class="grid">
  <div class="panel">
    <div class="panel-title">Kategoriradar</div>
  ...
  <div class="panel">
    ...
    <div class="score-row">
      ...
      <div class="score-bar"><div class="score-bar-fill" style="width:{{AverageValue20Pct}}%;background:{{#if (gt AverageValue 4)}}#22c55e{{else}}{{#if (gt AverageValue 3)}}#f59e0b{{else}}#ef4444{{/if}}{{/if}}"></div></div>
...
const avgs=cats.map(c=>{const a=allQ.filter(q=>q.c===c);return a.reduce((s,q)=>s+q.v,0)/a.length;});
new Chart(document.getElementById('r'),{type:'radar',data:{labels:cats,datasets:[{data:avgs,backgroundColor:'rgba(96,165,250,.15)',borderColor:'#60a5fa',pointBackgroundColor:'#60a5fa',borderWidth:2,pointRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{r:{min:0,max:5,ticks:{color:'#334155',stepSize:1,font:{size:8}},grid:{color:'#1e293b'},pointLabels:{color:'#94a3b8',font:{size:9}}}}}});
</script></body></html>`,
...
thead{background:#0f2d4a;color:white}th{padding:9px 12px;text-align:left}
td{padding:8px 12px;border-bottom:1px solid #e2e8f0}tr:nth-child(even){background:#f8fafc}
.notice{padding:13px;background:#fffbeb;border:1px solid #fcd34d;border-radius:7px;font-size:11px;color:#92400e}
...
{{QuestionsTableHtml}}
<div class="notice">ℹ Playwright-mall: Stödjer enbart <code>{{SurveyTitle}}</code>, <code>{{CompanyName}}</code>, <code>{{GeneratedAt}}</code>, <code>{{QuestionsTableHtml}}</code>. Inga loopar/Chart.js — använd jsreport (Hybrid A) för det.</div>
</body></html>`,
};
