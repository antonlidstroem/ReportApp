...
const props = defineProps<{ provider: string; supportsHtmlTemplate: boolean }>()
...
const templateOpts = computed(() => {
  ...
  return [
    ...
    { id: 'executive', icon: '🌙', name: 'Executive',           desc: 'Mörkt tema + kategoridiagram via Chart.js' },
    { id: 'custom',    icon: '✏️', name: 'Anpassad HTML',       desc: 'Använd Template Designer'                },
  ]
})
...
const lastResult = ref<{ ok: boolean; ms?: number; size?: string; fmt: string; tmpl: string; msg?: string } | null>(null)
...
async function doExport() {
  ...
  try {
    const qIds = store.selectedQuestionIds.length > 0 ? store.selectedQuestionIds.join(',') : undefined
    const r = await exportReport({
      provider: props.provider, format: selectedFmt.value, surveyId: store.surveyId,
      htmlTemplate: getTemplate(), start: store.dateStart || undefined,
      end: store.dateEnd || undefined, questionIds: qIds,
    })
    ...
    bench.add({ provider: props.provider, format: selectedFmt.value, surveyId: store.surveyId,
      surveyTitle: survey?.title ?? '', generationMs: r.generationMs, fileSizeBytes: r.fileSizeBytes,
      timestamp: new Date().toISOString(), questionCount: store.selectedQuestionIds.length || (survey?.questionCount ?? 0) })
  } catch (err) {
    lastResult.value = { ok: false, fmt: selectedFmt.value, tmpl,
      msg: err instanceof Error ? err.message.substring(0, 200) : 'Okänt fel' }
...
.tp-opt:last-child{border-bottom:none}.tp-opt:hover{background:rgba(255,255,255,.03)}
.tp-opt.active{background:color-mix(in srgb,var(--accent,#a855f7) 6%,transparent)}
.tp-opt input{display:none}.tp-icon{font-size:15px;flex-shrink:0;margin-top:1px}
...
.exp-btn:hover:not(:disabled){filter:brightness(1.1)}.exp-btn:disabled{opacity:.5;cursor:not-allowed}.exp-btn.loading{cursor:wait;opacity:.8}
...
.fade-enter-active,.fade-leave-active{transition:all .2s}.fade-enter-from,.fade-leave-to{opacity:0;transform:translateY(4px)}
</style>
