<template>
  <div class="template-designer">
    <div class="td-header">
      ...
      <div class="template-presets">
        <button
          ...
          :key="t.name"
          class="preset-btn"
          :class="{ active: activePreset === t.name }"
          @click="loadPreset(t)"
        >{{ t.name }}</button>
    ...
    <div class="note" v-if="!supportsHtmlTemplate">
      ⚠️ <strong>{{ providerName }}</strong> stödjer inte HTML-mallar (kod-first approach).
      Mallen visas för förhandsgranskning men används inte vid export.
    </div>

    <div class="td-body">
      <!-- Code editor -->
      <div class="editor-pane">
        <div class="pane-label">HTML-mall <span class="var-hint" @click="showVars = !showVars">{{ showVars ? '▲' : '▼' }} variabler</span></div>
...
const templateVars = [
  '{{SurveyTitle}}', '{{CompanyName}}', '{{GeneratedAt}}',
  '{{StartDate}}', '{{EndDate}}',
...
const MOCK: Record<string, unknown> = {
  SurveyTitle: 'Förhandsgranskning — Exempelrapport',
  CompanyName: 'Demo AB',
...
function renderTemplate(html: string, data: Record<string, unknown>): string {
  ...
  result = result.replace(eachReg, (_, key, body) => {
    ...
    return arr.map((item, idx) => {
      let row = body
      row = row.replace(/\{\{@index\}\}/g, String(idx + 1))
...
function loadPreset(t: { name: string; html: string }) {
  activePreset.value = t.name
  store.templateHtml = t.html
}
...
function formatHtml() {
  ...
  let level = 0
  const lines = store.templateHtml
    .replace(/></g, '>\n<')
    .split('\n')
    .map(l => l.trim())
    .filter(Boolean)
...
.save-btn { border-color: #22c55e; color: #22c55e; }
...
.var-list code:hover { background: var(--accent); color: white; }
...
.slide-enter-active, .slide-leave-active { transition: all 0.15s; }
