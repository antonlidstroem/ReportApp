  ...
  <div class="ie">
    <div class="ie-formats">
      <button
        ...
        @click="selectedFmt = f.id"
        :title="f.label"
      >{{ f.icon }}</button>
    </div>
    <button
      :class="['ie-btn', { loading: isLoading }]"
      :disabled="isLoading || !surveyId"
      @click="doExport"
    >
      <span v-if="!isLoading">⬇ Exportera {{ selectedFmt.toUpperCase() }}</span>
...
const props = defineProps<{
  ...
  htmlTemplate?: string          // Pre-built template to inject
  surveyIdOverride?: number      // If set, uses this survey; else uses store.surveyId
...
.ie-btn:hover:not(:disabled) { filter: brightness(1.12); }
.ie-btn:disabled { opacity: .5; cursor: not-allowed; }
.ie-btn.loading { cursor: wait; }
...
.ie-result.ok { color: #22c55e; background: rgba(34,197,94,.1); }
.ie-result.err { color: #ef4444; background: rgba(239,68,68,.1); max-width: 220px; }
</style>
