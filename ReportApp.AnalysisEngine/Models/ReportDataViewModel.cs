        ...
        public string QuestionsTableHtml
        {
            get
            {
                ...
                foreach (var q in QuestionSummaries)
                {
                    ...
                    sb.Append($"<td style=\"padding:7px 12px\">{System.Web.HttpUtility.HtmlEncode(q.Text)}</td>");
                    sb.Append($"<td style=\"padding:7px 12px\">{System.Web.HttpUtility.HtmlEncode(q.Category)}</td>");
